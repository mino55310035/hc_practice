...
import { checkIsToday } from "@/utils/dateHelpers"
    ...
    return (
        <div>
            <div className='flex items-center gap-2 text-gray-600'>
                <button onClick={handlePreviousWeek}>←前週</button>
                <button onClick={handleNextWeek}>次週→</button>
            </div>
                ...
                {days.map((date, index)=>(
                    ...
                    onClick={() => {
                        ...
                        {getEventsForDate(date).slice(0, 3).map((event)=>(
                            <div key={event.id} className="bg-blue-500 p-0.5 rounded-b-sm mt-1 text-white text-left text-xs"
                            onClick={(e) => {
                                e.stopPropagation();
                                handleOpenEditClick();
